...
export function MainFooter() {
  ...
  return (
    <footer className="bg-slate-950 text-white border-t border-slate-800">
      <div className="py-10 border-b border-slate-800 bg-slate-900/30">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-8">
            <h3 className="text-lg font-semibold text-white mb-2">Enterprise-Grade Security & Compliance</h3>
            <p className="text-sm text-slate-400">Trusted by Fortune 500 companies worldwide</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {certifications.map((cert) => (
              <div key={cert.name} className="flex flex-col items-center p-5 rounded-lg bg-slate-900 border border-slate-800 hover:border-cyan-500/50 transition-all group">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <cert.icon className="w-6 h-6 text-white" />
                </div>
                <span className="text-sm font-bold text-white mb-1">{cert.name}</span>
                <span className="text-xs text-slate-400 text-center">{cert.description}</span>
              </div>
            ))}
          </div>
        </div>
