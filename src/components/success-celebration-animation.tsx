...
export default function SuccessCelebration({ isVisible, onComplete }: { isVisible: boolean; onComplete: () => void }) {
      ...
      {isVisible && (
        ...
        >
          ...
          >
            ...
            <motion.h2 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="text-4xl md:text-5xl font-bold text-white mb-2">
              {message.title}
            </motion.h2>

            <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="text-lg text-slate-200 max-w-md mx-auto">
              {message.subtitle}
            </motion.p>

            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5, type: 'spring' }} className="mt-6 flex justify-center gap-2">
              {[Star, Sparkles, Star].map((Icon, i) => (
                <motion.div
                  key={i}
                  animate={{ rotate: [0, 15, -15, 0], scale: [1, 1.2, 1] }}
                  transition={{ duration: 0.6, delay: 0.6 + i * 0.1, repeat: Infinity, repeatDelay: 1 }}
                >
